...
$main = document.querySelector("main"), //var type DOM
$links = document.querySelector(".links"); //var type DOM

async function loadCharacter(url) {
    try {
        $main.innerHTML = `<img class="loader" src="../assets/img/loader.svg" alt="Loading...">`;
        ...
        if(!response.ok) throw { status:response.status,statusText:response.statusText } //catch error
      ...
      $prevLink = json.info.prev ? `<a href="${json.info.prev}">Previous ⏪</a>`:"";
